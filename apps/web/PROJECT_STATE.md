...
- [x] 1-b (tooling: TS, ESLint, Prettier, Vitest, Playwright; web tsconfig updated with JSX/ESNext/interop; package.json patched; **TS path alias `@egc/calc-core` added**)
...
5. calc-core package
- [x] 5-a (**types kept minimal and pure**; app-specific buckets moved to web store; entrypoint `src/index.ts` added)
- [x] 5-b (functions: `compute` implemented; **helpers `gross`, `net` added**; deterministic rounding)
- [ ] 5-c
- [ ] 5-d
...
- [x] 7-a (inputs wired with Zustand store; SaleAmountsSection built; **store refactored to local `InputBuckets` + adapter ? calc-core**)
- [ ] 7-b
- [~] 7-c (**compute() remains pure**; perf benchmarking pending)
- [x] 7-d (permanent “ESTIMATION” badge in CalculatorClient; **Net vs Goal progress bar added**)
...
- [~] 22-a (partial) *Done:* repo setup, **calc-core entrypoint + helpers**, TS alias `@egc/calc-core`, env validation, CSV UI scaffold, mapping wizard integrated. *Pending:* Neon DB initialization, rollups posting, worker parse fix.
- [~] 22-b (partial) *Done:* **calculatorStore refactor (local types + adapter to core)**, CalculatorClient tabs scaffold, ESTIMATION badge, **Net vs Goal progress bar**, calc client import fixes. *Pending:* remaining input sections, goal tracking UX, CSV variance integration.
- [ ] 22-c
- [ ] 22-d
- [ ] 22-e
...
Monorepo and tooling in place. `calc-core` is **pure** and exports `compute` and lightweight helpers (`gross`, `net`) via a package entrypoint, resolved by the `@egc/calc-core` alias. The web app defines UI-specific buckets and adapts them to core inputs in a Zustand store. CalculatorClient has tabs scaffolded and a Net vs Goal progress indicator. Mapping wizard and Recharts demo are integrated on `/csv`. CSV parsing in the Worker is still blocked, which delays rollup preview and charts.

---
## 4) Next Actions (actionable queue)
0) **Incorporate all math calculations** — ensure all math is coded, tested, and ready for scaling when we add the API.  
...
5) **Add remaining dashboard sections (7-b)** — Shipping, COGS, Fees, Ads, Payments, Cross-border, Taxes, Refunds, Store, Attribution.  
6) **Rollups posting** — POST summarized rollups to worker API. Owner: Joe
